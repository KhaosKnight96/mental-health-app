...
<style>
    ...
    [data-testid="stChatMessageUser"] { background-color: #1E3A8A !important; }
    ...
    .stButton>button { border-radius: 10px; font-weight: 600; width: 100%; height: 3em; }
</style>
...
try:
    ...
    client = Groq(api_key=st.secrets.get("GROQ_API_KEY", "MISSING"))
...
with tabs[0]:
    ...
    with st.expander("⚡ Sync Energy Status"):
        val = st.select_slider("Energy Level", options=list(range(1,12)), value=6, key="esl")
        if st.button("Log Energy", key="ebt"):
            log_to_sheet("Sheet1", {"timestamp": datetime.now().strftime("%Y-%m-%d %H:%M"), "memberid": st.session_state.auth['mid'], "energylog": val})
            st.toast("Saved!")
    
    for msg in st.session_state.chats["Cooper"]: st.chat_message(msg["role"]).write(msg["content"])
    if pr := st.chat_input("Chat with Cooper...", key="c_chat"):
        st.session_state.chats["Cooper"].append({"role": "user", "content": pr})
        log_to_sheet("ChatLogs", {"timestamp": datetime.now(), "memberid": st.session_state.auth['mid'], "agent": "Cooper", "role": "user", "content": pr})
        try:
...
with tabs[1]:
    ...
    for msg in st.session_state.chats["Clara"]: st.chat_message(msg["role"]).write(msg["content"])
    if pr_c := st.chat_input("Chat with Clara...", key="cl_chat"):
        st.session_state.chats["Clara"].append({"role": "user", "content": pr_c})
        log_to_sheet("ChatLogs", {"timestamp": datetime.now(), "memberid": st.session_state.auth['mid'], "agent": "Clara", "role": "user", "content": pr_c})
        try:
...
with tabs[2]:
    ...
    if st.session_state.active_game is None:
        ...
        if st.button("⬅️ Exit Game", key="exit_g"): st.session_state.active_game = None; st.rerun()
        
        if st.session_state.active_game == "Snake":
            st.components.v1.html(f"<script>{SOUND_JS}</script>" + """
            <div style="background:#1E293B; padding:10px; border-radius:15px; display:flex; flex-direction:column; align-items:center; touch-action:none;">
                <canvas id="snk" width="280" height="280" style="background:#0F172A; border-radius:10px; border:1px solid #38BDF8;"></canvas>
                <h4 id="score" style="color:#38BDF8;">Score: 0</h4>
            </div>
            <script>
                const cvs=document.getElementById("snk"), ctx=cvs.getContext("2d");
                ...
                function rst(){ snk=[{x:140,y:140}]; fd={x:Math.floor(Math.random()*20)*b,y:Math.floor(Math.random()*20)*b}; d=null; clearInterval(gm); gm=setInterval(drw,120); }
                function drw(){
                    ...
                    if(h.x==fd.x&&h.y==fd.y){ fd={x:Math.floor(Math.random()*20)*b,y:Math.floor(Math.random()*20)*b}; playSfx(440,0.1,'square'); } else if(d)snk.pop();
                    if(h.x<0||h.x>=280||h.y<0||h.y>=280||(d&&snk.some(s=>s.x==h.x&&s.y==h.y))){ playSfx(100,0.4,'sawtooth'); rst(); }
                    if(d)snk.unshift(h); document.getElementById("score").innerText="Score: "+(snk.length-1);
                }
                rst();
            </script>""", height=420)
            ...
            <div style="background:#1E293B; padding:20px; border-radius:15px; text-align:center;">
                ...
                <div id="mgrid" style="display:grid; grid-template-columns:repeat(3,70px); gap:10px; margin:20px auto; width:230px;"></div>
                <button id="mbtn" style="background:#F472B6; color:white; border:none; padding:10px 20px; border-radius:8px; width:100%;">Start Round</button>
            </div>
            <script>
                let l=1, s=[], u=[], w=false; const g=document.getElementById("mgrid");
                ...
                async function tap(i){
                    if(w) return; await flash(i,"#38BDF8"); u.push(i);
                    ...
                    else if(u.length===s.length){ l++; document.getElementById("mlvl").innerText="Level "+l; w=true; playSfx(800,0.2); }
                }
                build();
            </script>""", height=420)
            ...
            st.components.v1.html(f"<script>{SOUND_JS}</script>" + """
            <div id="gbox" style="background:#1E293B; padding:15px; border-radius:15px; display:flex; flex-direction:column; align-items:center; touch-action:none;">
...
# --- 8. ADMIN ---
with tabs[3]:
    if st.session_state.auth['role'] != "admin": st.warning("Admins Only")
    else:
        df_log = load_sheet("ChatLogs")
        if not df_log.empty:
            u_flt = st.multiselect("Filter Users", df_log['memberid'].unique())
            res_df = df_log[df_log['memberid'].isin(u_flt)] if u_flt else df_log
