...
if "zen_mode" not in st.session_state: st.session_state.zen_mode = None
...
mode = st.session_state.zen_mode if st.session_state.zen_mode else main_nav
...
if mode == "Dashboard":
    ...
    with col1:
        ...
        if st.button("Log Energy", use_container_width=True, type="primary"):
            ...
            st.balloons()
            st.success(f"Logged {score_text}!")
