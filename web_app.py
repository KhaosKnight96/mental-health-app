...
with st.sidebar:
    ...
    st.divider()
    if st.button("🚪 Logout"):
        # Clear all session state to prevent KeyErrors on re-entry
...
if st.session_state.get("current_page") == "Dashboard":
    ...
    with col1:
        ...
        if not udf.empty:
            try:
                ...
                if not row.empty:
                    pb = pd.to_numeric(row['HighScore'].values[0], errors='coerce') or 0
            except: pb = 0
    ...
    with col2:
        ...
        if p_in:
            ...
            msgs = [{"role":"system","content":f"You are Cooper, companion for {cname}."}] + \
    ...
    else: st.info("No logs found for this Couple ID.")

# --- 7. GAMES ---
# (Memory Match and Snake HTML blocks as before...)
