...
def save_log(agent, role, content):
    try:
        # Removed 'sentiment' column from the log entry
...
def get_ai_response(agent, prompt, history):
    try:
        ...
        if agent == "Cooper":
            sys_prompt = (
                "You are Cooper, a trusted and reliable male friend. Your vibe is steady, loyal, and supportive. "
                "You speak like a real person, not an assistant. You give solid advice when asked, but mostly "
                "you're just there to listen and have your friend's back. Speak with warmth and maturity."
            )
        else: # Clara
            sys_prompt = (
                "You are Clara, a close female friend. Your speech is fluid, natural, and very human-like. "
                "Use modern, casual language (like 'totally', 'I feel you', 'honestly') and remain deeply empathetic. "
                "You are a friend chatting, not a master or a therapist. Be expressive and keep it flowing."
            )

        full_history = [{"role": "system", "content": sys_prompt}] + history[-8:] + [{"role": "user", "content": prompt}]
        res = client.chat.completions.create(
            model="llama-3.3-70b-versatile", 
            messages=full_history,
            temperature=0.8 # Higher temp for more natural/fluid speech
        )
...
# --- 6. ARCADE (MOBILE SWIPE + KEYBOARD) ---
with tabs[2]:
    game = st.selectbox("Choose a Game", ["Snake", "Memory Match", "Simon Says"])
    if game == "Snake":
        st.components.v1.html("""<div style="text-align:center; background:#1E293B; color:white; padding:15px; border-radius:20px; font-family:sans-serif;"><div id="scr">Score: 0</div><canvas id="snk" width="300" height="300" style="background:#0F172A; border:2px solid #38BDF8; margin:10px auto; display:block;"></canvas></div><script>const c=document.getElementById('snk'), x=c.getContext('2d'); let sn=[{x:150,y:150}], f={x:75,y:75}, d='R', sc=0, sx, sy; window.onkeydown=e=>{ if(e.key=='ArrowUp'&&d!='D')d='U'; if(e.key=='ArrowDown'&&d!='U')d='D'; if(e.key=='ArrowLeft'&&d!='R')d='L'; if(e.key=='ArrowRight'&&d!='L')d='R'; }; c.addEventListener('touchstart', e=>{sx=e.touches[0].clientX; sy=e.touches[0].clientY;}, false); c.addEventListener('touchend', e=>{ let dx=e.changedTouches[0].clientX-sx, dy=e.changedTouches[0].clientY-sy; if(Math.abs(dx)>Math.abs(dy)){ if(dx>30&&d!='L')d='R'; else if(dx<-30&&d!='R')d='L'; } else { if(dy>30&&d!='U')d='D'; else if(dy<-30&&d!='D')d='U'; } }, false); setInterval(()=>{ x.fillStyle='#0F172A'; x.fillRect(0,0,300,300); x.fillStyle='#F87171'; x.fillRect(f.x,f.y,15,15); x.fillStyle='#38BDF8'; sn.forEach(p=>x.fillRect(p.x,p.y,15,15)); let h={...sn[0]}; if(d=='L')h.x-=15; if(d=='U')h.y-=15; if(d=='R')h.x+=15; if(d=='D')h.y+=15; if(h.x==f.x&&h.y==f.y){sc++; f={x:Math.floor(Math.random()*19)*15,y:Math.floor(Math.random()*19)*15}} else sn.pop(); if(h.x<0||h.x>=300||h.y<0||h.y>=300){sn=[{x:150,y:150}]; sc=0; d='R';} sn.unshift(h); document.getElementById('scr').innerText="Score: "+sc; }, 130);</script>""", height=420)
...
# --- 7. REFINED ADMIN (DELETE USER HISTORY RESTORED) ---
with tabs[3]:
    if st.session_state.auth["role"] == "admin":
        st.subheader("🛡️ Admin Log Management")
        ...
        if not l_df.empty:
            ...
            with st.expander("🔍 Filter & Search", expanded=True):
                ...
                f_user = c1.selectbox("User ID", ["All Users"] + list(l_df['memberid'].unique()))
                f_agent = c2.selectbox("Agent", ["All Agents", "Cooper", "Clara"])
                f_date = c3.date_input("Date Range", [date(2025, 1, 1), date.today()])
                f_text = c4.text_input("Message Content")

            filt = l_df.copy()
            filt['timestamp_dt'] = filt['timestamp_fixed'].dt.date
            
            if f_user != "All Users": filt = filt[filt['memberid'].astype(str) == str(f_user)]
            if f_agent != "All Agents": filt = filt[filt['agent'] == f_agent.capitalize()]
            if len(f_date) == 2: filt = filt[(filt['timestamp_dt'] >= f_date[0]) & (filt['timestamp_dt'] <= f_date[1])]
            if f_text: filt = filt[filt['content'].str.contains(f_text, case=False, na=False)]

            st.write(f"Total entries: {len(filt)}")
            # Removed 'sentiment' from display
            display_df = filt.drop(columns=['timestamp_dt', 'timestamp_fixed'])
            st.dataframe(display_df.sort_values('timestamp', ascending=False), use_container_width=True, hide_index=True)

            # --- DATA DELETION SECTION ---
            st.divider()
            st.error("⚠️ DATA PURGE")
            target_user = st.selectbox("Select User to Purge History:", ["None"] + list(l_df['memberid'].unique()))
            
            if st.button("🔥 Permanently Delete Selected User History"):
                if target_user != "None":
                    # Keep all rows except the target user
                    new_logs = l_df[l_df['memberid'].astype(str) != str(target_user)]
                    ...
                    st.cache_data.clear()
                    st.success(f"All history for user '{target_user}' has been deleted.")
                    st.rerun()
                else:
                    st.warning("Please select a valid User ID.")
        else: st.info("Database is currently empty.")
    else: st.error("Unauthorized: Admin credentials required.")
