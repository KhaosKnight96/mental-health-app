...
if not st.session_state.auth["logged_in"]:
    ...
    if st.button("Enter Dashboard", use_container_width=True):
        try:
            ...
            st.error("Could not connect to the User Database. Check your 'Users' tab name.")
    st.stop()
...
# --- 5. AI HANDLERS (Filtered by CoupleID) ---
def handle_patient_chat():
    ...
    if user_text:
        ...
        for m in st.session_state.chat_log[-6:]:
            msgs.append({"role": "user" if m["user_type"] == "Patient" else "assistant", "content": m["content"]})
...
def handle_caregiver_chat(energy_df):
    ...
    if user_text:
        # Context is ONLY from this couple's logs and chats
        p_chat = "\n".join([f"{m['user_type']}: {m['content']}" for m in st.session_state.chat_log][-10:])
        e_data = energy_df.tail(7).to_string()
        
        prompt = f"System: You are Clara, a health analyst for {couple_name}. Context:\nChats: {p_chat}\nLogs: {e_data}"
...
if "Patient" in role_choice:
    st.title(f"Welcome, Cooper is here.")
    score = st.select_slider("How is your energy today?", options=range(1,11), value=5)
    ...
    if st.button("Save Daily Status"):
        try:
            ...
            st.success(f"Log saved for household {couple_id}!")
