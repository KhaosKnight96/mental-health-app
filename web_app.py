...
# Helper to fetch and clean the User DataFrame
def get_clean_users():
    df = conn.read(worksheet="Users", ttl=0)
    # Force column names to be clean: "Username", "Password", "Fullname", "HighScore"
    df.columns = [str(c).strip().title().replace(" ", "") for c in df.columns]
    # Ensure HighScore is numeric
    if 'Highscore' in df.columns:
        df = df.rename(columns={'Highscore': 'HighScore'})
    return df
...
if st.session_state.current_page == "Dashboard":
    ...
    with col1:
        # High Score Card with Error Handling
        udf = get_clean_users()
        try:
            val = udf.loc[udf['Username'].astype(str) == cid, 'HighScore'].values[0]
            pb = pd.to_numeric(val, errors='coerce') or 0
        except: pb = 0
...
# --- 7. GAMES (HTML/JS) ---
# (Memory Match and Snake HTML blocks go here as before)
