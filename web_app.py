...
# --- 5. THE ARCADE (WITH SCORE BRIDGE) ---
with tabs[2]:
    game_mode = st.radio("Select Activity", ["Snake", "2048", "Memory Pattern", "Flash Match"], horizontal=True)
    
    # 🟢 NEW: Score Capture Logic
    # We use a custom component 'st_bridge' style approach
    def record_score(game, score):
        if score > 0:
            save_log("Arcade", "system", f"GAME_SCORE: {game} | Score: {score}")
            st.toast(f"🏆 {game} Score of {score} logged!")
    ...
    // THE BRIDGE: This sends data back to Streamlit
    function sendScoreToStreamlit(score) {
    ...
    if game_mode == "Snake":
        # We capture the returned value from the HTML component
        score_data = st.components.v1.html(JS_CORE + """
            <div class="game-container">
                ...
                <div id="sn-go" class="overlay" style="display:none;">
                    <h1 style="color:#F87171;">GAME OVER</h1>
                    <button class="game-btn" onclick="sn_init()">Play Again</button>
                </div>
            </div>
            <script>
            let sn, f, d, g, s, run=false; 
            const c=document.getElementById("sn-c"), x=c.getContext("2d"), b=15;
            
            function sn_init(){ 
                sn=[{x:150,y:150}]; f={x:150,y:75}; d="R"; s=0; run=true; 
                document.getElementById("sn-go").style.display="none"; 
                clearInterval(g); g=setInterval(sn_upd,120); 
            }
            ...
            function sn_upd(){
                ...
                if(h.x<0||h.x>=300||h.y<0||h.y>=300||sn.some(z=>z.x==h.x&&z.y==h.y)){
                    run=false; clearInterval(g); snd(100,"sawtooth",0.5);
                    document.getElementById("sn-go").style.display="flex";
                    // 🟢 BRIDGE TRIGGER: Send score to Python when snake dies
                    sendScoreToStreamlit(s);
                }
                if(run)sn.unshift(h);
                document.getElementById("sn-s").innerText=s;
            }
            sn_init();
            </script>
            <style>
                .game-container { text-align:center; background:#1E293B; padding:20px; border-radius:20px; font-family:sans-serif; }
                .overlay { display:flex; position:absolute; top:0; left:0; width:100%; height:100%; background:rgba(15,23,42,0.9); flex-direction:column; align-items:center; justify-content:center; }
                .game-btn { padding:12px 25px; background:#38BDF8; border:none; border-radius:8px; color:white; cursor:pointer; }
                .score-board { color:#38BDF8; font-size:20px; margin-bottom:10px; }
            </style>
        """, height=500)

        # 🟢 PYTHON SIDE: Check if score_data has changed
        if score_data is not None:
            record_score("Snake", score_data)
