...
def get_ai_response(agent, prompt, history):
    try:
        ...
        sys = f"You are {agent}, a trusted friend. Speak human, not like an AI."
...
with tabs[3]:
    ...
    with col_a:
        st.write("### Search for Friends")
        search_id = st.text_input("Enter Member ID to add:").strip().lower()
        if st.button("Send Friend Request"):
            if search_id == st.session_state.auth['mid']:
                st.error("You cannot add yourself.")
            elif search_id not in u_df['memberid'].astype(str).values:
                st.error("User not found.")
            else:
                # Check if request exists
                exists = not f_df[((f_df['sender'] == st.session_state.auth['mid']) & (f_df['receiver'] == search_id)) | 
                                 ((f_df['sender'] == search_id) & (f_df['receiver'] == st.session_state.auth['mid']))].empty
                if exists:
                    st.warning("Request already pending or you are already friends.")
                else:
                    new_req = pd.DataFrame([{"sender": st.session_state.auth['mid'], "receiver": search_id, "status": "pending"}])
                    conn.update(worksheet="Friends", data=pd.concat([f_df, new_req], ignore_index=True))
                    st.success(f"Request sent to {search_id}!")
                    st.cache_data.clear()

    with col_b:
        st.write("### Incoming Requests")
        incoming = f_df[(f_df['receiver'] == st.session_state.auth['mid']) & (f_df['status'] == "pending")]
        if incoming.empty: st.info("No pending requests.")
        for _, req in incoming.iterrows():
            c1, c2 = st.columns([2, 1])
            c1.write(f"Request from **{req['sender']}**")
            if c2.button("Accept", key=f"acc_{req['sender']}"):
                f_df.loc[(f_df['sender'] == req['sender']) & (f_df['receiver'] == st.session_state.auth['mid']), 'status'] = "accepted"
...
with tabs[4]:
    ...
    if not friends_list:
        st.warning("You need to add and accept friends before you can message them.")
    else:
        sel_f = st.selectbox("Chat with Friend:", friends_list)
        ...
        if p_msg := st.chat_input("Message..."):
